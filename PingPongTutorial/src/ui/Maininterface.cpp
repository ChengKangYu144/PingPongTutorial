...
MainInterface::MainInterface() : Interface(L"Main")
{
	...
	_pOnePlayerButton->SetUpStyle(RED)->SetHoverStyle(YELLOW)
		->SetDownStyle(MAGENTA)->SetOnClick(_OnClickOnePlayerButton);
	
	_pTwoPlayerButton = new Button(Rect(550, 280, 100, 60), L"2 Player");
	_pTwoPlayerButton->SetUpStyle(RED)->SetHoverStyle(YELLOW)
		->SetDownStyle(MAGENTA)->SetOnClick(_OnClickTwoPlayerButton);

	_pExitButton = new Button(Rect(900, 460, 60, 60), L"Exit");
	_pExitButton->SetUpStyle(RED)->SetHoverStyle(YELLOW)
		->SetDownStyle(MAGENTA)->SetOnClick(_OnClicExitButton);
}
